...
import AllContactEntries from "../../components/dashboard/AllContactEntries";
...
export default function Page() {
    ...
    const fetchData = async () => {
        console.log(Bearer);
        const response = await fetch(`${baseUrl}info/getAllContactEntries/`, {
            method: "POST",
            headers: {
    ...
    return (
        <div className="flex flex-col h-screen w-full gap-0">
            <TopHeadline heading="Contact Requests" />
            ...
            <div className="flex justify-between items-center px-8 py-4">
                <ResultsCount
                    count={filteredResults.length > 0 ? filteredResults.length : data.length}
                />
            </div>
            <div className="flex flex-col justify-center items-center px-8">
                {filteredResults.length > 0 ? (
                    filteredResults.map((item, index) => (
                        <AllContactEntries key={index} item={item}/>
                    ))
                ) : (
                    data.map((item, index) => (
                        <AllContactEntries key={index} item={item} />
