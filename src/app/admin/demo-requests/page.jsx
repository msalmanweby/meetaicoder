...
export default function Page() {
    ...
    const fetchData = async () => {
        console.log(Bearer);
        const response = await fetch(`${baseUrl}info/getAllDemoEntries/`, {
            method: "POST",
            headers: {
    ...
    function handleSearch(data, companyName) {
        ...
        return data.filter(item => item.company.toLowerCase().includes(searchQuery));
    }
            ...
            <div className="flex flex-col justify-center items-center px-8">
                {filteredResults.length > 0 ? (
                    filteredResults.map((item, index) => (
                        <AllEntries key={index} item={item}/>
                    ))
                ) : (
                    data.map((item, index) => (
                        <AllEntries key={index} item={item} />
