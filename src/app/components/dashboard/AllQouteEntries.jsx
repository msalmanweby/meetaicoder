...
function AllQouteEntries({ item }) {
  ...
  return (
    <>
      ...
      <div
        className={`overflow-hidden w-full transition-max-height duration-500 ease-linear no-scrollbar ${
      ...
      >
        <div className="flex flex-col p-4 items-start bg-gray-950 border-[0.5px] border-gray-900 rounded-[10px] text-white text-[16px] font-medium">
          ...
          <p className="capitalize">Country: {item.country}</p>
          <p>Email: {item.email}</p>
          <p>Phone Number: {item.phone_number}</p>
          <p>Company: {item.company}</p>
          <p className="capitalize">
            Categories: {item.project_category.join(", ")}
          </p>
          <p>Reference Link: {item.link}</p>
...
export default AllQouteEntries;
