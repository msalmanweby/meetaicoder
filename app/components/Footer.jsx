  ...
  return (
    <div>
      <div className='flex items-start p-4 text-white justify-between border-t-[1px] border-gray-900'>
        <div className='hidden sm:flex'><Image src="/logo.svg" width={264} height={36} alt='Meet-Ai Coder' /></div>
        <div className='flex flex-col gap-2 p-2'>
          <h1 className='sm:text-xl text-gray-300'>Overview</h1>
          {MenuItems.map((item, index) => (
            <Link className='text-sm font-normal text-gray-500 hover:text-gray-300' key={index} href={item.href}>{item.title}</Link>
          ))}
        </div>
        <div className='flex flex-col gap-2 p-2'>
